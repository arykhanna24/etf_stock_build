...
sector_info <- read.csv("/Users/aryan/Documents/etf_analysis/import/nasdaq_screener_1705895340252.csv") %>%
  clean_names() %>% 
  filter(country == "United States") %>% 
  select(ticker = symbol,sector,industry)


combined_build <- stock_build_one %>% 
  inner_join(pe_ratio_build, by = c("ticker")) %>% 
  left_join(sector_info, by = "ticker") %>% 
  select(1:3,35:41) %>% 
  mutate(pe_ratio = as.double(pe_ratio)) %>% 
  group_by(sector,industry) %>% 
  mutate(
    percentile_rank = rank(pe_ratio)/length(pe_ratio)*100,
    avg = mean(pe_ratio,na.rm = T)
  ) %>% 
  ungroup() %>% 
  filter(percentile_rank < 30,pe_ratio < avg) %>% 
  arrange(sector,industry,percentile_rank) %>% 
  left_join(s_and_p_500,by = "ticker") %>% 
  filter(sp500  == 1)
   

s_and_p_500 <- read.csv("/Users/aryan/s_and_p_constituents.csv") %>% 
  transmute(ticker = symbol,sp500 = 1)
